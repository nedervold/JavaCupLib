...
/** CUP v0.11b beta 20140226 generated parser.
  * @version Mon Jul 21 13:45:58 PDT 2014
  */
public class parser extends java_cup.runtime.lr_parser {
 ...
 /* override error routines */
  ...
  public void report_fatal_error(
    ...
    {
      done_parsing();
      if (info instanceof Symbol) ErrorManagerAccess.getManager().emit_fatal(message+ "\nCan't recover from previous error(s), giving up.",(Symbol)info);
      else ErrorManagerAccess.getManager().emit_fatal(message + "\nCan't recover from previous error(s), giving up.",cur_token);
      System.exit(1);
    }
    ...
    public void report_error(String message, Object info)
    {
      if (info instanceof Symbol)
         ErrorManagerAccess.getManager().emit_error(message,(Symbol)info);
      else
         ErrorManagerAccess.getManager().emit_error(message,cur_token);
    }
...
// TUM CHANGES
  ...
  protected Hashtable<String, production_part> symbols = new Hashtable<String, production_part>();
    ...
    throws java.lang.Exception
    {
      ...
      switch (CUP$parser$act_num)
		...
		RESULT = start_val;
          ...
          symbols.put("error", new symbol_part(parser.terminalFactory.error));

          /* declare start non terminal */
          non_terms.put("$START", parser.nonTerminalFactory.START_nt());
	  ...
	  Emitter emit = parser.emitter;
	  /* save the package name */
	  emit.set_package_name(multipart_name);
	  ...
	  if (emit.action_code() != null)
	    ErrorManagerAccess.getManager().emit_warning("Redundant action code (skipping)");
	  else /* save the user included code string */
	    emit.set_action_code(user_code);
	  ...
	  if (emit.parser_code()!=null)
	    ErrorManagerAccess.getManager().emit_warning("Redundant parser code (skipping)");
	  else /* save the user included code string */
	    emit.set_parser_code(user_code);
	  ...
	  if (emit.init_code()!=null)
	    ErrorManagerAccess.getManager().emit_warning("Redundant init code (skipping)");
	  else /* save the user code */
	    emit.set_init_code(user_code);
	  ...
	  if (emit.scan_code()!=null)
	    ErrorManagerAccess.getManager().emit_warning("Redundant scan code (skipping)");
	  else /* save the user code */
	    emit.set_scan_code(user_code);
	  ...
	  if (symbols.get(sym) == null)
	    {
	      /* issue a message */
	      ErrorManagerAccess.getManager().emit_error("Terminal \"" + sym + 
	  ...
	  if (nt == null)
	    {
	    ErrorManagerAccess.getManager().emit_error( "Start non terminal \"" + start_name + 
	  		         "\" has not been declared");
	    }
          else
	    {
	      ...
	      add_rhs_part(new symbol_part(parser.terminalFactory.EOF));
	      if (!emit.xmlactions()) add_rhs_part(new action_part("RESULT = start_val;"));
	      emit.set_start_production(
		     parser.productionFactory.createProduction(parser.nonTerminalFactory.START_nt(), rhs_parts, rhs_pos));
	      new_rhs();
	    }
	  ...
	  if (lhs_nt == null)
	    {
	      ...
	      ErrorManagerAccess.getManager().emit_warning("LHS non terminal \"" + lhs_id + 
			       "\" has not been declared");
	    }
	  ...
	  new_rhs();
		...
		String lhs_id = (String)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-4)).value;
          ...
          case 60: // NT$12 ::= 
            {
              Object RESULT =null;
 ErrorManagerAccess.getManager().emit_error("Syntax Error"); 
	  ...
	  if (lhs_nt != null) 
		...
		p = parser.productionFactory.createProduction(lhs_nt, rhs_parts, rhs_pos,
              ...
              if (start_nt == null)
		{
		  ...
		  if ((sym!=null) && (sym instanceof terminal)) {
		    emit.set_start_production(
		      parser.productionFactory.createProduction(parser.nonTerminalFactory.START_nt(), rhs_parts, 
	  ...
	  if (lhs_nt != null) 
	    {
	      /* build the production */
	      production p = parser.productionFactory.createProduction(lhs_nt, rhs_parts, rhs_pos);
	  ...
	  if (symb == null)
	    {
	      if (ErrorManagerAccess.getManager().getErrorCount() == 0)
	        ErrorManagerAccess.getManager().emit_error("java_cup.runtime.Symbol \"" + symid + 
			       "\" has not been declared");
	    }
	  else
	    {
 ...
 multipart_names.push(multipart_name); multipart_name="";
	  ...
	  if (symbols.get(term_id) != null)
	    {
	      /* issue a message */
	      ErrorManagerAccess.getManager().emit_error("java_cup.runtime.Symbol \"" + term_id + 
		...
		multipart_name = "Object";
	      ...
	      symbols.put(term_id, 
	        new symbol_part(parser.terminalFactory.createTerminal(term_id, multipart_name)));
	    }
	  ...
	  if (symbols.get(non_term_id) != null)
	    {
	      /* issue a message */
	      ErrorManagerAccess.getManager().emit_error( "java_cup.runtime.Symbol \"" + non_term_id + 
			                      "\" has already been declared");
	    }
	  else
	    {
		...
		parser.nonTerminalFactory.createNonTerminal(non_term_id, multipart_name);
		...
		ErrorManagerAccess.getManager().emit_error("Illegal use of reserved word");
		RESULT="ILLEGAL";
