...
public class Main {
	...
	protected static void parse_args(String argv[]) {
		...
		for (i = 0; i < len; i++) {
			...
			if (argv[i].equals("-package")) {
				...
				/* record the name */
				emit.instance().package_name = argv[i];
				...
				/* record the name */
				emit.instance().parser_class_name = argv[i];
				...
				/* record the name */
				emit.instance().symbol_const_class_name = argv[i];
			...
			/* TUM changes; suggested by Henning Niss 20050628 */
				...
				/* record the typearg */
				emit.instance().class_type_argument = argv[i];
			}
	...
	protected static void open_files() {
		...
		/* parser class */
		out_name = emit.instance().parser_class_name + ".java";
		...
		out_name = emit.instance().symbol_const_class_name + ".java";
		fil = new File(dest_dir, out_name);
		try {
	...
	protected static void check_unused() {
		...
		for (Enumeration t = terminal.all(); t.hasMoreElements();) {
			...
			if (term.use_count() == 0) {
				...
				emit.instance().unused_term++;
				if (!emit.instance().nowarn) {
	...
	protected static void build_parser() throws internal_error {
		...
		start_state = lalr_state.build_machine(emit.instance().start_production);
		...
		if (emit.instance().num_conflicts > expect_conflicts) {
			ErrorManager.getManager().emit_error(
	...
	protected static void emit_parser() throws internal_error {
		...
		symbol_class_file.println("public " + class_or_interface + " "
				+ emit.instance().symbol_const_class_name + " {");
		...
		emit.instance().symbols_time = System.currentTimeMillis() - start_time;
		emit.instance().parser(parser_class_file, action_table, reduce_table,
				start_state.index(), emit.instance().start_production, opt_compact_red,
				suppress_scanner);
	}
	...
	protected static void emit_summary(boolean output_produced) {
		...
		/* unused symbols */
		System.err.println("  " + emit.instance().unused_term + " terminal"
				+ plural(emit.instance().unused_term) + " declared but not used.");
		System.err.println("  " + emit.instance().unused_non_term + " non-terminal"
				+ plural(emit.instance().unused_term) + " declared but not used.");

		/* productions that didn't reduce */
		System.err.println("  " + emit.instance().not_reduced + " production"
				+ plural(emit.instance().not_reduced) + " never reduced.");

		/* conflicts */
		System.err.println("  " + emit.instance().num_conflicts + " conflict"
				+ plural(emit.instance().num_conflicts) + " detected" + " ("
				+ expect_conflicts + " expected).");

		/* code location */
		if (output_produced)
			System.err.println("  Code written to \"" + emit.instance().parser_class_name
					+ ".java\", and \"" + emit.instance().symbol_const_class_name
					+ ".java\".");
		else
	...
	protected static void show_times() {
		...
		if (emit.instance().symbols_time != 0)
			System.err.println("        Symbols      "
					+ timestr(emit.instance().symbols_time, total_time));
		if (emit.instance().parser_time != 0)
			System.err.println("        Parser class "
					+ timestr(emit.instance().parser_time, total_time));
		if (emit.instance().action_code_time != 0)
			System.err.println("          Actions    "
					+ timestr(emit.instance().action_code_time, total_time));
		if (emit.instance().production_table_time != 0)
			System.err.println("          Prod table "
					+ timestr(emit.instance().production_table_time, total_time));
		if (emit.instance().action_table_time != 0)
			System.err.println("          Action tab "
					+ timestr(emit.instance().action_table_time, total_time));
		if (emit.instance().goto_table_time != 0)
			System.err.println("          Reduce tab "
					+ timestr(emit.instance().goto_table_time, total_time));
