...
public class Main {
	...
	protected static void parse_args(String argv[]) {
		...
		for (i = 0; i < len; i++) {
			...
			if (argv[i].equals("-package")) {
				...
				/* record the name */
				emit.set_parser_class_name(argv[i]);
				...
				/* record the name */
				emit.set_symbol_const_class_name(argv[i]);
			...
			/* TUM changes; suggested by Henning Niss 20050628 */
				...
				/* record the typearg */
				emit.set_class_type_argument(argv[i]);
			}
	...
	protected static void emit_parser() throws internal_error {
		...
		emit.set_symbols_time(System.currentTimeMillis() - start_time);
